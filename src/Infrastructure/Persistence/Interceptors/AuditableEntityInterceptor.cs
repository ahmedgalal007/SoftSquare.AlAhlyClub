...
namespace SoftSquare.AlAhlyClub.Infrastructure.Persistence.Interceptors;
...
public class AuditableEntityInterceptor : SaveChangesInterceptor
{
        ...
        foreach (var entry in context.ChangeTracker.Entries<BaseAuditableEntity>())
            switch (entry.State)
            {
                case EntityState.Added:
                    ...
                    if (entry.Entity is IMustHaveTenant mustTenant) mustTenant.TenantId = tenantId;
                    if (entry.Entity is IMayHaveTenant mayTenant) mayTenant.TenantId = tenantId;
                    break;
    ...
    private async Task TryUpdateTemporaryPropertiesForAuditTrail(DbContext context,
        CancellationToken cancellationToken = default)
    {
